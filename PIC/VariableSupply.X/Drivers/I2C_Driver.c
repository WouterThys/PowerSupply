 ...
 *          MACRO FUNCTIONS
 ******************************************************************************/
#define i2cAddressWrite(addr)   I2CTRN = (((addr << 1) & I2C_ADDRESS_MASK) + 0)
#define i2cAddressRead(addr)    I2CTRN = (((addr << 1) & I2C_ADDRESS_MASK) + 1)
#define i2cDataWrite(data)      I2CTRN = data
#define i2cDataRead()           I2CRCV
#define i2cStart()              I2CCONbits.SEN = 1
#define i2cRepeatStart()        I2CCONbits.RSEN = 1
#define i2cReceiveEn()          I2CCONbits.RCEN = 1
#define i2cAckEn()              I2CCONbits.ACKEN = 1
#define i2cAck()                I2CCONbits.ACKDT = 0
#define i2cNAck()               I2CCONbits.ACKDT = 1
#define i2cStop()               I2CCONbits.PEN = 1
#define i2cSclRel()             I2CCONbits.SCLREL = 1
#define i2cCheck(msk)           ((I2CSTAT & msk) > 0)
    ...
    if (mstrAddr) {                         // Master has addressed this device
        ...
        if (!mstrWrite) {                   // Multiple byte read
            ...
            if (fsm->dataCnt == 1) {        // Command byte
                ...
                // Handle I2C
                p->bufferAdr = i2cDataRead(); // Set the pointer
                p->command = p->bufferAdr;
                wMSB = false;
                rMSB = false;
...
void i2cDriverInit() {
    ...
    /* Interrupts master */
    MI2CIF = 0; // Clear flag
    MI2CIP = IP_I2C; // Priority is highest
    MI2CIE = 1; // Enable
}
#endif
...
void i2cDriverInit(i2cPackage_t *data, void (*onI2cEvent)(i2cPackage_t data)) {
    ...
    /* Interrupts slave */
    SI2CIF = 0; // Clear flag
    SI2CIP = IP_I2C; // Priority is highest
    SI2CIE = 1; // Enable
}
#endif
